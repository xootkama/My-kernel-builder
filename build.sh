...
setversioning() {
    # For staging branch
    KERNELNAME="$NAMA-$VARIAN-$JENIS-$LINUXVER-$DATE"
...
exports() {
    ...
    export KBUILD_BUILD_VERSION="50"
	export ARCH=arm64
...
gen_zip() {
	...
	cd AnyKernel3 || exit
        cp -af "$KERNEL_DIR"/init.WizardSpectrum.rc init.spectrum.rc && sed -i "s/persist.spectrum.kernel.*/persist.spectrum.kernel Tiktodz/g" init.spectrum.rc
