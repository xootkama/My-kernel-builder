...
exports() {
    ...
    export KBUILD_BUILD_VERSION="805"
	export ARCH=arm64
...
gen_zip() {
	...
	cd AnyKernel3 || exit
	cp -af "$KERNEL_DIR"/init.ElectroSpectrum.rc init.spectrum.rc && sed -i "s/persist.spectrum.kernel.*/persist.spectrum.kernel WizardPrjkt/g" init.spectrum.rc
