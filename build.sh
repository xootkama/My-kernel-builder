...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
	...
	then
		...
		git clone --depth=1 https://github.com/BENKz29/venom-clang.git -b master clang

	elif [ $COMPILER = "gcc" ]
	then
		msg "|| Cloning GCC  ||"
		git clone --depth=1 https://github.com/arter97/arm64-gcc gcc64 $KERNEL_DIR/gcc64
		git clone --depth=1 https://github.com/arter97/arm32-gcc gcc32 $KERNEL_DIR/gcc32
...
gen_zip() {
	...
	cd AnyKernel3 || exit
	cp -af "$KERNEL_DIR"/init.WizardSpectrum.rc init.spectrum.rc && sed -i "s/persist.spectrum.kernel.*/persist.spectrum.kernel WizardPrjkt/g" init.spectrum.rc
