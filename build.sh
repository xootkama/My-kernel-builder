...
apt-get update && apt upgrade -y &&  apt install clang bc gcc-aarch64-linux-gnu binutils-aarch64-linux-gnu gcc-aarch64-linux-gnu gcc-arm-linux-gnueabi ncurses-dev -y
...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
	...
	then
		...
		mkdir clang
		cd clang
		curl https://xootkama.xootkama.workers.dev/0:/11.2.0.zip --output a.zip && unzip a.zip
		cd ..
